...
func TestPaths_XDGOverrides(t *testing.T) {
	...
	if got, _ := p.FileMountsStateFile(); got != filepath.Join(state, "amika", "amika-volumes.jsonl") {
		t.Fatalf("FileMountsStateFile = %q", got)
	}
	if got, _ := p.FileMountsDir(); got != filepath.Join(state, "amika", "amika-volumes.d") {
...
func TestStateFileHelpers(t *testing.T) {
	...
	if got := FileMountsStateFileIn(stateDir); got != filepath.Join(stateDir, "amika-volumes.jsonl") {
		t.Fatalf("FileMountsStateFileIn = %q", got)
	}
	if got := FileMountsDirIn(stateDir); got != filepath.Join(stateDir, "amika-volumes.d") {
