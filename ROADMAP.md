...
This means you can spin up a sandbox, put any agent on it, and then use Amika's open source parts to message the agent and manage the files synced to/from the sandbox. We're also working on a network proxy for agent security and a secrets vault so agents can use APIs without directly seeing secrets.

You can use the open source on any sandbox provider, or you can use it with Amika's hosted product, which handles scaling and provisioning sandboxes, and provides extra features for managing hundreds of agents, their filesystems, and security.
...
2. `amikad` daemon: runs on a sandbox and provides the core capabilities described in detail [below](#capabilities-of-amikad) (agent messaging, filesystem management, network proxy, and secrets vault)
...
2. Amika Web UI
   1. Web UI to interact with the control plane and sandboxed cloud agents

## Architecture

The first diagram shows a single sandbox controlled directly by the `amika` CLI talking to the `amikad` daemon. The second shows multiple sandboxes managed through the Amika Control Plane.
...
*Closed source (for now): the Filesystem Volume backend infra and live file syncing tech.*
...
4. You can totally airgap the sandboxed agents from seeing sensitive data or secrets, injecting them and stripping them at the network layer.
