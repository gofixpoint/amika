...
RUN /tmp/amika-step.sh && rm /tmp/amika-step.sh

COPY sandbox-image/assets/stable /tmp/amika-stable-assets
COPY sandbox-image/steps/20-static-config.sh /tmp/amika-step.sh
RUN /tmp/amika-step.sh /tmp/amika-stable-assets \
    && rm -rf /tmp/amika-step.sh /tmp/amika-stable-assets
...
COPY sandbox-image/assets/hooks /tmp/amika-hook-assets
COPY sandbox-image/steps/70-hook-assets.sh /tmp/amika-step.sh
RUN /tmp/amika-step.sh /tmp/amika-hook-assets \
    && rm -rf /tmp/amika-step.sh /tmp/amika-hook-assets
...
RUN AMIKA_PRESET=coder-dind /tmp/amika-step.sh && rm /tmp/amika-step.sh
