...
The canonical skill tree is installed at `~/.agents/skills`. opencode and pi
load that path themselves. Claude Code does not, so the step also links the
Amika skill into `~/.claude/skills/amika-cli` (`image.agent_skill_link`); that
harness reads `~/.agents/skills` only as an import source, and loads user skills
from `~/.claude/skills` alone. Linking the individual skill leaves the directory
available for Claude-only skills from a base image or later setup. A link rather
than a second copy means one skill cannot drift from itself.
