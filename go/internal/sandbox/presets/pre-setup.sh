...
# Created as root; ownership is fixed by the find below.
...
# Hand anything root-owned in the home directory back to the amika user, e.g.
# intermediate directories left root-owned by Docker bind-mounts (agent
# credential files under ~/.local/). Filtering with find instead of running a
# blanket `chown -R` skips files that already have the right owner, avoiding
# ctime churn and overlayfs copy-ups on large home directories.
find /home/amika \( ! -user amika -o ! -group amika \) -exec chown -h amika:amika {} +