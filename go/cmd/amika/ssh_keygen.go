...
func runSSHKeygen(cmd *cobra.Command, paths basedir.Paths, name string, format output.Format) error {
	...
	// Order matters in both directions, because the upload and the local SSH
	// config have to end up describing the same keypair. Validate before the
	// upload, then persist only after the upload succeeds.
